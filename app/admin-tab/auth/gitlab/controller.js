...
export default Ember.Controller.extend({
  ...
  wasRestrictedMsg: function() {
    var users = this.get('originalModel.allowedIdentities').filterBy('externalIdType',C.PROJECT.TYPE_GITLAB_USER).get('length');
    var orgs = this.get('originalModel.allowedIdentities').filterBy('externalIdType',C.PROJECT.TYPE_GITLAB_ORG).get('length');
    var enterprise = !!this.get('originalModel.hostname');

    var gitlab = 'GitLab' + ( enterprise ? ' Enterprise' : '');

    var str = 'project members';
    if ( users )
    {
      str += (orgs ? ', ' : ' and ') +  users + ' ' + gitlab + ' user' + (users === 1 ? '' : 's');
    }

    if ( orgs )
    {
      str += ' and ' + orgs + (users ? '' : ' ' + gitlab) + ' organization' + ( orgs === 1 ? '' : 's');
    }
  ...
  actions: {
    test: function() {
      ...
      this.set('gitlab.hostname', model.get('hostname'));
      this.set('gitlab.clientId', model.get('clientId'));
    ...
    authenticate: function() {
      this.send('clearError');
      this.get('gitlab').authorizeTest((err,code) => {
        if ( err )
        {
          this.send('gotError', err);
        }
